...
const buildInfo = () => {
	...
	let names = escapeInvalidCharacters(
		`${rd.querySelector('h1').textContent.trim()}[/]${rd.querySelector('.subtitle').textContent.trim()}`,
	);
...
iframe.addEventListener('load', () => {
	const dataSrc = (iframe.contentDocument.querySelector('img.lazy') || iframe.contentDocument.querySelector('img#gimg'))
		.src;
	console.log(iframe.contentDocument.querySelector('img#gimg'), dataSrc);
	...
	const myWindow = window.open('', '', `width=${w_width},height=${w_height},left=${w_left},top=${w_top}`);
	...
	downlink.addEventListener('click', () => {
		myWindow.document.querySelector('textarea').value = iframe.dataset.textToCop.replace(
			'[imgs]',
			optimizeImageUrlsString(getStringSrc(getArraySrc(myWindow.document))),
		);
	});
